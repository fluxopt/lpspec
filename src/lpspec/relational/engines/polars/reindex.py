...
Both take the :class:`~lpspec.relational.engines.polars.space.Space` and
...
class _Order:
    ...
    @classmethod
    def of(cls, space: Space, dimension: str, partition: program.Walk | None) -> _Order:
        ...
        """
        grouping = Grouping.whole(space.data, dimension) if partition is None else Grouping.of(space.data, partition)
...
def window_fragment(space: Space, p: TermFragment, s: program.Window, context: str) -> TermFragment:
    ...
    order = _Order.of(space, s.dimension, s.partition)

    width_name = s.width if isinstance(s.width, str) else None
    if width_name is not None:
        widest = int(space.data.parameters[width_name].select(pl.col('value').max()).collect().item() or 0)
    else:
        assert not isinstance(s.width, str)
        widest = s.width
    lags = pl.LazyFrame({_LAG: pl.Series(range(min(widest, space.data.cardinality[s.dimension])), dtype=pl.Int64)})
    ...
    def lagged(frame: pl.LazyFrame) -> pl.LazyFrame:
        ...
        if width_name is None:
            return frame
        widths, keys = _named_amount(space, order, width_name, _WIDTH)
    ...
    def travelled(presence: Presence) -> Presence:
        ...
        if keyed_by is not None and not set(order.grouping.keys).issubset(keyed_by):
            source, keyed_by = space.widen(source, keyed_by, p.dims), None
...
def translate_fragment(space: Space, p: TermFragment, s: program.Translate, context: str) -> TermFragment:
    ...
    def travelled(presence: Presence) -> Presence:
        ...
        vacated = edge.vacated_of(space, presence, p.dims)
    ...
    if not s.wrap and s.fill is not None and p.kind == 'const':
        frame = pl.concat([frame, edge.filled(space, others, s.fill)], how='vertical_relaxed')
...
class _Edge:
    ...
    @classmethod
    def of(cls, space: Space, order: _Order, s: program.Translate) -> _Edge:
        if not isinstance(s.offset, str):
            return cls(order, s, (), None)
        dims = space.program.parameter(s.offset).dims
        offset_dims = tuple(d for d in dims if order.grouping.column_of(d) is None)
        return cls(order, s, offset_dims, _named_amount(space, order, s.offset, _OFFSET))
    ...
    def filled(self, space: Space, others: list[str], fill: float) -> pl.LazyFrame:
        ...
        for d in others:
            if d in self.keys:
                continue
            edge = edge.join(space.data.dimensions[d].select(pl.col('val').alias(d)), how='cross')
        return edge.with_columns(pl.lit(fill, dtype=pl.Float64).alias('cval')).select(
            *others, self.shift.dimension, 'cval'
        )

    def vacated_of(self, space: Space, presence: Presence, dims: tuple[str, ...]) -> pl.LazyFrame:
        ...
        source = presence.frame if all(d in have for d in others) else space.widen(presence.frame, have, dims)
...
def _named_amount(space: Space, order: _Order, name: str, alias: str) -> tuple[pl.LazyFrame, list[str]]:
    ...
    reaches everywhere else.
    """
    dims = space.program.parameter(name).dims
    keys = [order.grouping.column_of(d) or d for d in dims]
    frame = space.data.parameters[name].select(
