...
from lpspec.relational.engines.polars.fragments import GROUP_RANK, GROUP_SIZE, group_columns
...
def compile_predicate(
    ...
    **A name the mask is certain of is joined rather than left-joined**,
    ...
    def join_group_offset(p: program.DimensionPositionNode, walk: program.Walk) -> str:
        """One column: the row's ordinal minus its own group's target ordinal."""
        for d in (p.name, *walk.joined_dims):
    ...
    def join_lookup(lookup: str, column: str | None) -> str:
        """The lookup's *column* read at the frame's coordinates of its key — or, with no column, whether a row is there.

        A keyed lookup is read at its key; a bare relation, which only a bare
        name reads, at every column. The frame carries those dimensions, or
        the language refused the mask.
        """
        lk = compiler.program.lookups[lookup]
        keyed = lk.key or lk.roles
        dims = [lk.dim(r) for r in keyed]
        assert len(set(dims)) == len(dims), f"lookup '{lookup}' is read at two columns over one dimension"
        for d in dims:
            refuse_outside_foreach(f"lookup '{lookup}' read at dimension '{d}'", d)
        read = pl.col(column) if column is not None else pl.lit(value=True)
        return carrier.once(
            f'__where lookup {lookup}.{column or ""}__',
            lambda f, alias: f.join(
                compiler.data.lookups[lookup].select(
                    *(pl.col(r).alias(d) for r, d in zip(keyed, dims, strict=True)), read.alias(alias)
                ),
                on=dims,
                how='left',
            ),
    ...
    def walk(p: program.WhereNode) -> pl.Expr:
        ...
        if isinstance(p, program.DimensionPositionNode):
            if p.partition is not None:
                offset = pl.col(join_group_offset(p, p.partition))
                return falsy_if_null(_COLUMN_COMPARISONS[p.op](offset, pl.lit(0)))
...
def _refuse_short_groups(p: program.DimensionPositionNode, walk: program.Walk, table: pl.LazyFrame) -> None:
    ...
    *table* is :meth:`PolarsCompiler.partitioned`'s, so a coordinate in no
    ...
    is named by its value tuple, with the joined coordinate it sits at beside.
    """
    needed = p.position + 1 if p.position >= 0 else -p.position
    columns = group_columns(walk)
    sizes = table.select(*columns, GROUP_SIZE).unique().collect()
    short = sorted(str(key[0] if len(columns) == 1 else tuple(key)) for *key, n in sizes.iter_rows() if n < needed)
    if short:
