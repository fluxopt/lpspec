...
class PolarsEngine:
    ...
    def build(self, program: program.Program, sources: Mapping[str, pl.LazyFrame]) -> None:
        ...
        **A second call rebuilds over the same object**, which is what
        ...
        is why the held solver is asked to
        :meth:`~lpspec.relational.sinks.solvers.base.Solver.digest` what it
        loaded first: it is holding those frames, and sixteen bytes is all it
        needs of them.
