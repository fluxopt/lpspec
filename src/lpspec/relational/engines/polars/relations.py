...
def mapping(relations: Mapping[str, pl.LazyFrame], walk: program.Walk) -> pl.LazyFrame:
    ...
    Consumed and joined columns come under their dimensions, produced ones
    under :func:`landing`. A key the relation does not map has no row, which
    is what "reaches no slot" means.
    """
    return relations[walk.name].select(
...
def walk_join(
    ...
    keeps every dimension of *have* but the consumed ones, gains the produced
    ones under their own names, and keeps *columns* beside them. A walk brings
    the dimensions it lands on, so the produced ones are never among those
    kept — except where the consumed and the produced are one dimension, which
    a self-map does, so the two are traded in a single select.
    ...
    """
    consumed = node.walk.consumed_dims
    gained = node.walk.produced_dims
    keep = [d for d in have if d not in consumed]
    traded = frame.join(mapping, on=[*consumed, *node.joined], how='inner').select(
