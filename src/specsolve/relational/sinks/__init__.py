...
from specsolve.relational.sinks import sos
...
def relaxations(program: program.Program, name: str) -> list[str]:
    ...
    In :data:`~specsolve.relational.sinks.capabilities.CAPABILITIES` order.
    """
    table = sink_capabilities(name)
    needed = caps.required(program)
    return [
...
def ingestible(name: str, tables: Tables, program: program.Program | None = None) -> Tables:
    ...
    :func:`~specsolve.relational.sinks.sos.reformulated` tables, so everything that
    ...
    *program* is what the refusal is decided on, and is optional. Given one, a
    ...
    Raises:
        SpecsolveError: A *program* carrying a construct this sink has no concept
