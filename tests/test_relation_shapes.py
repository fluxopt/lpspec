...
#: `f3` has no row, so neither column of the tuple it would read exists.
READING_OF = pl.DataFrame({'flow': ['f1', 'f2'], 'component': ['c1', 'c1'], 'kind': ['k1', 'k1']})

DANGLING_PAIR: dict[str, Any] = {
    'dimensions': {'flow': {'dtype': 'str'}, 'component': {'dtype': 'str'}, 'kind': {'dtype': 'str'}},
    'relations': {'reading_of': {'key': 'flow', 'values': ['component', 'kind']}},
    'variables': {
        'level': {'dims': ['component', 'kind'], 'bounds': {'lower': 0, 'upper': 10}},
        'take': {'dims': ['flow'], 'bounds': {'lower': 0, 'upper': 10}},
    },
    'constraints': {
        'link': {'dims': ['flow'], 'expression': 'take <= at(level, by=reading_of, over=[component, kind], into=flow)'}
    },
    'objective': {
        'sense': 'maximize',
...
def test_a_pullback_reading_a_two_column_slot_that_does_not_exist_takes_its_row_with_it():
    """A key the relation leaves out reads nothing, so its row is not built.

    A lane that reported neither the relation's own gap nor the operand's would
    build `take[f3] <= 0` and pin a flow the model never spoke about.
    """
    sources = {'flow': ['f1', 'f2', 'f3'], 'component': ['c1', 'c2'], 'kind': ['k1'], 'reading_of': READING_OF}
    with lps.build(DANGLING_PAIR, sources) as model:
        result = model.solve()
        assert result.objective == pytest.approx(10.0, rel=RTOL), (
            'take[f3] is held by its own bound, not by a row reading nothing'
        )
        assert model.diagnostics().rows == 2, 'the two flows whose tuple maps have a row, and f3 has none'
