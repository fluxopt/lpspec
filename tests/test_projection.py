...
from specsolve.projection import _hull
...
def hull(region: sps.Region) -> list[tuple[float, float]]:
    return [tuple(row) for row in region.hull.select(region.x, region.y).rows()]


def piece(region: sps.Region, i: int) -> list[tuple[float, float]]:
    return [tuple(row) for row in region.vertices.filter(pl.col('piece') == i).select(region.x, region.y).rows()]


def bound_by(region: sps.Region, i: int, edge: int) -> list[tuple[str, str, str]]:
...
def test_a_region_one_axis_direction_cannot_see_is_found_by_refinement():
    ...
    finds ``(4, 2)`` and ``(2, 4)``."""
    region = sps.project(CORNER, CORNER_SOURCES, x='a', y='b', at={'t': 0})
...
def test_a_spec_without_an_objective_has_no_optimum():
    ...
    region = sps.project(spec, CORNER_SOURCES, x='a', y='b', at={'t': 0})
...
def test_without_at_a_quantity_is_summed_over_every_dim():
    ...
    region = sps.project(CORNER, CORNER_SOURCES, x='a', y='b')
...
def test_a_scalar_expression_is_an_axis_as_it_stands():
    ...
    region = sps.project(CORNER, CORNER_SOURCES, x='total_a', y='b')
    assert hull(region) == [(0, 0), (5, 0), (5, 6), (3, 8), (0, 8)], (
...
def test_the_probe_stays_on_the_fast_path(monkeypatch: pytest.MonkeyPatch):
    ...
    from specsolve import projection
    ...
    sps.project(CORNER, CORNER_SOURCES, x='a', y='b', at={'t': 0})
    (model,) = seen
...
def test_at_on_a_scalar_quantity_is_refused():
    with pytest.raises(SpecsolveError, match="'total_a' does not carry: it is read over no dims"):
        sps.project(CORNER, CORNER_SOURCES, x='total_a', y='b', at={'t': 0})
...
def test_at_naming_a_dim_the_quantity_does_not_carry_is_refused():
    ...
    with pytest.raises(
        SpecsolveError, match="at names \\['t'\\], which 'b' does not carry: it is read over \\['unit'\\]"
    ):
        sps.project(spec, sources, x='a', y='b', at={'t': 0})


def test_at_naming_no_declared_dimension_is_refused():
    with pytest.raises(SpecsolveError, match="at names \\['hour'\\], which the spec declares no dimension for"):
        sps.project(CORNER, CORNER_SOURCES, x='a', y='b', at={'hour': 0})
...
def test_an_unknown_quantity_names_the_declared_ones():
    with pytest.raises(KeyError, match="unknown variable or named expression 'c'"):
        sps.project(CORNER, CORNER_SOURCES, x='a', y='c')


def test_the_same_quantity_on_both_axes_is_refused():
    with pytest.raises(SpecsolveError, match="both axes are 'a'"):
        sps.project(CORNER, CORNER_SOURCES, x='a', y='a')
...
def test_a_name_the_probe_adds_is_refused_where_the_spec_declares_it():
    ...
    with pytest.raises(SpecsolveError, match='already declares objective_weight under parameters:'):
        sps.project(spec, {**CORNER_SOURCES, 'objective_weight': 1.0}, x='a', y='b')


def test_a_lowered_program_is_refused():
    with pytest.raises(SpecsolveError, match='not a Program'):
        sps.project(sps.check(CORNER), CORNER_SOURCES, x='a', y='b')
...
def test_a_trace_that_never_settles_stops_rather_than_running_on(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(SpecsolveError, match='for 4 solves without settling'):
        sps.project(CORNER, CORNER_SOURCES, x='a', y='b', at={'t': 0})
...
def test_each_combination_of_the_binaries_is_its_own_piece():
    ...
    region = sps.project(committed(), CORNER_SOURCES, x='a', y='b', at={'t': 0}, binaries='each')
...
def test_free_binaries_give_the_hull_the_pieces_fill():
    free = sps.project(committed(), CORNER_SOURCES, x='a', y='b', at={'t': 0})
    each = sps.project(committed(), CORNER_SOURCES, x='a', y='b', at={'t': 0}, binaries='each')
...
def test_labels_say_what_varies_between_the_pieces():
    ...
    at_one_hour = sps.project(committed(), CORNER_SOURCES, x='a', y='b', at={'t': 0}, binaries='each')
    assert [at_one_hour.label(i) for i in range(2)] == ['on=0', 'on=1'], 'nothing but the value varies'
    over_both = sps.project(committed(), CORNER_SOURCES, x='a', y='b', binaries='each')
...
def test_an_infeasible_combination_is_left_out():
    ...
    region = sps.project(spec, CORNER_SOURCES, x='a', y='b', binaries='each')
...
def test_an_infeasible_model_has_no_pieces_either():
    ...
    with pytest.raises(NoSolutionError, match='the model is infeasible'):
        sps.project(spec, CORNER_SOURCES, x='a', y='b', binaries='each')
...
def test_the_pieces_move_on_the_fast_path(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(projection, 'Model', Watched)
    sps.project(committed(), CORNER_SOURCES, x='a', y='b', binaries='each')
    (model,) = seen
...
def test_each_on_a_model_with_no_binary_is_refused():
    with pytest.raises(SpecsolveError, match='the spec declares none'):
        sps.project(CORNER, CORNER_SOURCES, x='a', y='b', binaries='each')
...
def test_more_binary_columns_than_the_cap_are_refused(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(
        SpecsolveError, match='every combination of 2 binary columns, which is 4 regions; the most it traces is 2'
    ):
        sps.project(committed(), CORNER_SOURCES, x='a', y='b', binaries='each')
...
def test_a_pin_name_the_spec_declares_is_refused():
    ...
    with pytest.raises(SpecsolveError, match='already declares on_at_least under parameters:'):
        sps.project(spec, {**CORNER_SOURCES, 'on_at_least': 0.0}, x='a', y='b', binaries='each')


def test_binaries_outside_the_two_words_is_refused():
    with pytest.raises(SpecsolveError, match="binaries is 'free' or 'each'"):
        sps.project(CORNER, CORNER_SOURCES, x='a', y='b', binaries='all')  # pyrefly: ignore[bad-argument-type] — the refusal under test
...
def _chp_plant() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    spec = override(
        ...
        **{
            'constraints.nodal_balance.expression': 'sum(gen, by=gen_bus, over=generator, into=bus) + sum(incidence * p, over=link) >= load',
...
def test_the_chp_plant_traces_the_region_brute_force_enumerates():
    ...
    region = sps.project(spec, sources, x='heat_out', y='elec_out')
...
def test_a_segment_is_a_line_and_a_point_a_marker():
    ...
    (line,) = traces(sps.project(segment, CORNER_SOURCES, x='a', y='b', at={'t': 0}).plot(optimum=False), mode='lines')
    assert line.fill is None and len(line.x) == 2, 'a segment is a line, not a fill'
    point = override(
        CORNER, **{'variables.a.bounds': {'lower': 2, 'upper': 2}, 'variables.b.bounds': {'lower': 3, 'upper': 3}}
    )
    figure = sps.project(point, CORNER_SOURCES, x='a', y='b', at={'t': 0}).plot(optimum=False)
...
def test_a_second_region_on_the_same_figure_takes_the_next_colours():
    pytest.importorskip('plotly')
    first = sps.project(CORNER, CORNER_SOURCES, x='a', y='b', at={'t': 0})
    second = sps.project(CORNER, CORNER_SOURCES, x='a', y='b', at={'t': 1})
...
def test_plot_without_plotly_names_the_extra(monkeypatch: pytest.MonkeyPatch):
    ...
    region = sps.project(CORNER, CORNER_SOURCES, x='a', y='b', at={'t': 0})
    with pytest.raises(ModuleNotFoundError, match=r'pip install "specsolve\[plot\]"'):
        region.plot()
