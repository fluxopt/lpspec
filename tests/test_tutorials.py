...
back when one of them lands wrong. ``docs/lifecycle.md`` aims them at
linopy's `fix` / `relax` / `remove`, and ``docs/region.md`` traces a plant's
feasible region and reads its edges. Every executed block is a real call, so a
signature change breaks this test rather than leaving a page that reads fine
and errors in a reader's session.
