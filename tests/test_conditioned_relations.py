...
One relation, one value column, and a key of more than one dimension: the map
...
at both key dimensions.

What needs two lanes to see:

**The condition rides the join.** Relationally the extra key column is one
more equi-join column; on the eager lane the map is an array over the key's
product, and grouping by a two-dimensional label array is what stands in for
the one-dimensional coordinate a plain map assigns.

**A pair the map leaves out.** With a plain map an unmapped label is a whole
label of one dimension, and the eager lane could drop it before grouping.
A conditioned map leaves out a *pair*, so nothing can be dropped along either
axis: the operand is masked at that pair instead, and the two lanes have to
agree that its terms land nowhere.
"""
...
# ---------------------------------------------------------------------------
# what a conditioned map is still not
# ---------------------------------------------------------------------------


def test_a_partition_by_a_conditioned_map_is_refused_with_the_rewrite_named():
    """Which group a coordinate is in would depend on a column the walk cannot read.

    A partitioned `shift` moves along one dimension inside each group, and a
    key of two columns makes the group depend on the other one — which the walk
    has no row to read. Refused at the door, so `check` and `build` refuse the
    same file.
    """
    spec = override(
        raw_of(SPEC),
        **{
            'constraints.zonal': {
                'dims': ['generator', 'period'],
                'expression': 'p <= shift(p, along=period, offset=1, edge=0, by=zone_of)',
            }
        },
    )
    with pytest.raises(LpspecError, match=r"'zone_of' is keyed by \['generator', 'period'\]"):
        lps.check(spec)


def test_a_relation_the_key_does_not_determine_is_still_refused():
    """Two value columns are two maps, and the message says to split them."""
    spec = override(
        raw_of(SPEC),
        **{
            'relations.zone_of': {'columns': ['generator', 'period', 'zone'], 'key': 'generator'},
            'constraints.zonal.expression': 'sum(p, by=zone_of, over=generator, into=zone) <= zone_cap',
        },
    )
    with pytest.raises(LpspecError, match='has 2 columns its key does not determine'):
        lps.check(spec)
...
def test_a_key_column_naming_no_label_is_refused():
    ...
    with pytest.raises(DataError, match=r"relation 'zone_of' maps 9, which are not labels of 'period'"):
        lps.solve(SPEC, sources)
