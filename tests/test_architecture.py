...
#: ``ArithmeticComparison`` is the resolved form of a where comparison between
#: expressions; lowering rewrites every one into an ``ExpressionComparison``
#: over program expressions.
NEVER_LOWERED = frozenset({'ArithmeticComparison'})
...
def test_no_never_lowered_node_survives_lowering():
    ...
    assert 'ExpressionComparison' in seen, 'the probe must reach the node lowering replaces them with'
...
def test_both_lanes_dispatch_on_every_plan_node():
    ...
    **Node kinds, not their fields.** A field census reads as stricter and is
    ...
    declared = {
        node.__name__ for union in (program.Expression, program.Predicate) for node in get_args(union)
    } - NEVER_LOWERED
