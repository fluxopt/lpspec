...
def test_a_scenario_sweep_is_an_archive_and_runs_again(
    ...
    assert study.answer.records.drop('run').equals(runs.records.drop('run'))
    assert study.answer.records['run'].unique().to_list() == ['study'], (
    ...
    assert again.records['objective'].to_list() == pytest.approx(runs.records['objective'].to_list()), (
...
def test_every_slice_of_a_sweep_names_the_model_it_answered(
    ...
    assert runs.records['spec_digest'].null_count() == 0, 'no slice is left without the document it answered'
    assert runs.records['spec_digest'].unique().to_list() == [alone.spec_digest], (
