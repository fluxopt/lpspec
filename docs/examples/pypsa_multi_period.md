...
The [multi-period](multi_period.md) model has no build years, no lifetimes, no
...
`coal` pays once and `wind` pays once, discounted. That is what
...
`overnight_cost` (`costs.py:119`). The port carries no `years` parameter.
