...
One rung of [the PyPSA corpus](https://math-spec.readthedocs.io/en/latest/examples/pypsa/#rung-9--multi-link): the file `pypsa.yaml` projected onto what this network builds, attached to that network, and held to what PyPSA solves it to.
...
=== "specsolve"
    ...
    with sps.solve('differential/pypsa/rungs/rung_09_multilink.yaml', sources) as solution:
