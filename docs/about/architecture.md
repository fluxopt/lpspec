...
| `GroupSum` | `sum(x, by=r, over=k, into=v)` | many-to-one | one inner join with the relation's table on the columns the walk consumes and joins on, the consumed dims traded for the produced ones; a bare relation fans a member out to every target |
| `At` | `at(x, by=r, over=v, into=k)` | one-to-one | the same table joined the other way, fanning out |
