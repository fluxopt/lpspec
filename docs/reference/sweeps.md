...
| **the model is parsed once** | `solve_over` validates it up front, so a model outside the language fails before the data is touched. Every worker is handed the lowered program, in this process or across one, so none reads the YAML or lowers it again |
