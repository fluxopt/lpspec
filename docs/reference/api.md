...
| `lps.evaluate(spec, sources)` | a spec of parameters and expressions, no variables: its named expressions read as arithmetic, with no solver; returns an `Evaluation` |
...
| `lps.Model` / `lps.Result` / `lps.Runs` / `lps.Evaluation` | the types the verbs hand back, importable so a wrapper can annotate its signature. The spec going *in* is `math_spec.Spec` |
...
result.evaluation.expression('co2')  # a named expression at the solution, over its own dims
result.evaluation.evaluate('sum(p * rate)')  # a quantity the file never named, same shape
...
| **computed quantities are `result.evaluation`** | the reading half, split from the solve: `result.evaluation` hands back an `Evaluation`, whose `expression(name)` and `evaluate(expr)` read a quantity the file declared or one it never named. The same object `lps.evaluate` returns for a variable-free spec. A closed result and a solve with no primal both refuse it — there is no point to evaluate at |
| **`expression` takes a declared name** | the value of a [named expression](https://math-spec.readthedocs.io/en/latest/reference/language/expressions/#named-expressions) at the solution, aggregated to its own dimensions; never an expression string, which is `evaluate`'s argument. An unknown name is a `KeyError` listing what is declared. It is compiled at the read, so unread expressions cost nothing |
...
| **an ad-hoc `evaluate` names nothing** | so it is not a *kind*: `to_parquet` does not write it and a sweep does not spill it. A declared expression is: `save` writes it under `expression/`. To keep a quantity, declare it under `expressions:` and read it by name |
