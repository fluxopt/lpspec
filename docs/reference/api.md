...
| **`to_parquet` writes every kind, `to_dataset` the variables** | `primal/<name>.parquet`, `dual/<name>.parquet`, `expression/<name>.parquet` — the kind directory because a constraint may carry a variable's name. A dual undefined by an integer variable and an expression this data cannot evaluate are left out, and `dual` and `expression` still say why. `to_dataset` is variables only: a dual or an expression in it would collide with a variable of the same name |
