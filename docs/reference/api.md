...
| **an undeclared expression names nothing** | so it is not a *kind*: `save` does not write it, a sweep does not spill it, and no bridge takes it — a bridge labels what it hands back by the name it was given, so `kind='expression'` takes a declared name and refuses an expression string. A declared expression is a kind: `save` writes it under `expression/` and every bridge reads it. To keep a quantity, or to label it, declare it under `expressions:` and read it by name |
